...
{
    public partial class ShaftPull : Form
    {
        ...
        public ShaftPull(UIShoporder sh)
        {
            InitializeComponent();
            db  = new Dataconnect();
            this.ui = sh;
        }
        private void ShaftPull_Load(object sender, EventArgs e)
        {
            Displyalreadyinput();
            GetMinandMax();
            CheckStatusInput();
        ...
        public async void Displyalreadyinput()
        {
            ...
            string strsql = "SELECT s.Shoporder, sp.SP_supply, sp.SP_lot, a.SP_first, a.SP_second, a.SP_third, a.SP_fourth, a.SP_fifth " +
            ...
            if (bp.Rows.Count > 0)
            {
                ...
                SupplierText.Text = row["SP_supply"] != DBNull.Value ? (string)row["SP_supply"] : "";
                ShaftText.Text = row["SP_lot"] != DBNull.Value ? (string)row["SP_lot"] : "";
                firstText.Text = row["SP_first"] != DBNull.Value ? Convert.ToDecimal(row["SP_first"]).ToString() : "";
                SecondText.Text = row["SP_second"] != DBNull.Value ? Convert.ToDecimal(row["SP_second"]).ToString() : "";
                thirdText.Text = row["SP_third"] != DBNull.Value ? Convert.ToDecimal(row["SP_third"]).ToString() : "";
                fourthText.Text = row["SP_fourth"] != DBNull.Value ? Convert.ToDecimal(row["SP_fourth"]).ToString() : "";
                FifthText.Text = row["SP_fifth"] != DBNull.Value ? Convert.ToDecimal(row["SP_fifth"]).ToString() : "";
            }
        ...
        private async void updatebtn_Click(object sender, EventArgs e)
        {
            if (ValidationForm())
            {
                ...
                else
                {
                    ...
                    string measurequery = "INSERT INTO ProdCon_ShopOrder_supplier(Shoporder, SP_supply, SP_lot, ShopOrderID) " +
                     "VALUES (@Shoporder, @SP_supply, @SP_lot, @ShopOrderID)";
                ...
                if (await db.CheckifExist(stractualData))
                {
                    string updatesqldata = " UPDATE ProdCon_ShopOrder_ActualData SET " +
                                           " SP_first =@SP_first, SP_second =@SP_second, SP_third =@SP_third, SP_fourth =@SP_fourth, SP_fifth =@SP_fifth " +
                                           " WHERE Shoporder =@Shoporder AND ShopOrderID =@ShopOrderID";
                    SqlParameter[] updatedataparameter =
                    {
                     new SqlParameter("@Shoporder", shopordersec),
                     new SqlParameter("@SP_first", string.IsNullOrWhiteSpace(firstText.Text) ? 0 : Convert.ToDouble(firstText.Text)),
                     new SqlParameter("@SP_second",string.IsNullOrWhiteSpace(SecondText.Text) ? 0 : Convert.ToDouble(SecondText.Text)),
                     new SqlParameter("@SP_third", string.IsNullOrWhiteSpace(thirdText.Text) ? 0 : Convert.ToDouble(thirdText.Text)),
                     new SqlParameter("@SP_fourth", string.IsNullOrWhiteSpace(fourthText.Text) ? 0 : Convert.ToDouble(fourthText.Text)),
                     new SqlParameter("@SP_fifth", string.IsNullOrWhiteSpace(FifthText.Text) ? 0 : Convert.ToDouble(FifthText.Text)),
                     new SqlParameter("@ShopOrderID", ID)
                 };
                ...
                else
                {
                    ...
                    string dataquery = "INSERT INTO ProdCon_ShopOrder_ActualData(Shoporder, SP_first, SP_second, SP_third, SP_fourth, SP_fifth, ShopOrderID) " +
                     "VALUES (@Shoporder, @SP_first, @SP_second, @SP_third, @SP_fourth, @SP_fifth, @ShopOrderID)";
